...
export class TallyHub extends EventEmitter {
  ...
  private saveMixerConfigurations(): void {
    try {
      ...
      console.log(`💾 Saved ${mixersToSave.length} mixer configuration(s) to JSON`);
    } catch (error) {
      console.error('❌ Error saving mixer configurations:', error);
    }
  }

  private saveMixerConfigurationsToEnv(): void {
    try {
      ...
      // Remove existing mixer configurations from .env
      const envLines = envContent.split('\n');
      const filteredLines = envLines.filter(line => {
        const trimmedLine = line.trim();
        return !(
          trimmedLine.startsWith('OBS_HOST=') ||
          trimmedLine.startsWith('OBS_PORT=') ||
          trimmedLine.startsWith('OBS_PASSWORD=') ||
          trimmedLine.startsWith('VMIX_HOST=') ||
          trimmedLine.startsWith('VMIX_PORT=') ||
          trimmedLine.startsWith('# Mixer Configuration') ||
          trimmedLine.match(/^(OBS|VMIX)_.*=/)
        );
      });
      
      // Add header for mixer configurations
      filteredLines.push('');
      filteredLines.push('# Mixer Configuration');
      filteredLines.push('# Generated automatically - do not edit manually');
      
      // Add each mixer configuration
      const mixers = Array.from(this.mixerConnections.values());
      
      if (mixers.length === 0) {
        filteredLines.push('# No mixers configured');
      } else {
        for (const mixer of mixers) {
          filteredLines.push('');
          filteredLines.push(`# ${mixer.name} (${mixer.type})`);
          
          if (mixer.type === 'obs') {
            filteredLines.push(`OBS_HOST=${mixer.host}`);
            filteredLines.push(`OBS_PORT=${mixer.port}`);
            // Add password comment but don't include actual password for security
            filteredLines.push('# OBS_PASSWORD=your_password_here');
          } else if (mixer.type === 'vmix') {
            filteredLines.push(`VMIX_HOST=${mixer.host}`);
            filteredLines.push(`VMIX_PORT=${mixer.port}`);
          }
        }
      }
      ...
      console.log(`💾 Saved ${mixers.length} mixer configuration(s) to .env file`);
      console.log(`📄 .env file location: ${envPath}`);
      
    } catch (error) {
      console.error('❌ Error saving mixer configurations to .env:', error);
    }
  }

  public saveConfigurations(): void {
    this.saveMixerConfigurations();
    this.saveMixerConfigurationsToEnv();
  }
  ...
  private async checkTCPConnection(host: string, port: number, timeout = 3000): Promise<void> {
  ...
  private async connectToMixer(id: string, connection: MixerConnection): Promise<void> {
    ...
    // Preflight TCP check for vMix
    if (connection.type === 'vmix') {
      try {
        await this.checkTCPConnection(connection.host, connection.port, 3000);
        console.log(`✅ TCP connection to vMix at ${connection.host}:${connection.port} succeeded`);
      } catch (tcpError) {
        const msg = `❌ Cannot reach vMix at ${connection.host}:${connection.port} - ${tcpError instanceof Error ? tcpError.message : tcpError}`;
        connection.lastError = msg;
        console.error(msg);
        console.error('💡 vMix troubleshooting:');
        console.error('   • Ensure vMix is running');
        console.error('   • Enable TCP API in vMix (Settings > Web Controller > Enable TCP API)');
        console.error('   • Check host and port');
        console.error('   • Check Windows Firewall and network connectivity');
        throw tcpError;
      }
    }
    ...
    mixer.on('error', (error: Error) => {
      ...
      if (connection.type === 'vmix') {
        console.error('💡 vMix connection troubleshooting:');
        console.error('   • Ensure vMix is running');
        console.error('   • Enable TCP API in vMix (Settings > Web Controller > Enable TCP API)');
        console.error(`   • Check host: ${connection.host} and port: ${connection.port}`);
        console.error('   • Check Windows Firewall and network connectivity');
      }
    ...
    mixer.on('reconnect:failed', () => {
      ...
      if (connection.type === 'obs') {
        ...
        console.log(`   • Enable TCP API in vMix (Settings > Web Controller > Enable TCP API)`);
        console.log(`   • Check host: ${connection.host} and port: ${connection.port}`);
        console.log('   • Check Windows Firewall and network connectivity');
      }
    });
    ...
    // Add connection timeout with proper error handling
    let isTimedOut = false;
    const connectTimeout = setTimeout(() => {
      isTimedOut = true;
      console.error(`🕒 Connection timeout after 10 seconds for ${connection.name} at ${connection.host}:${connection.port}`);
      connection.lastError = `Connection timeout after 10 seconds`;
      if (connection.type === 'vmix') {
        console.error('💡 vMix connection troubleshooting:');
        console.error('   • Ensure vMix is running');
        console.error('   • Enable TCP API in vMix (Settings > Web Controller > Enable TCP API)');
    ...
    try {
      ...
      // Don't re-throw timeout errors to prevent server crashes
      if (isTimedOut) {
        console.error(`❌ Connection to ${connection.name} timed out and failed`);
  ...
  public addMixerConnection(id: string, connection: MixerConnection, password?: string): boolean {
    try {
      ...
      // Save configurations to both JSON and .env file
