...
export class WebSocketManager {
  ...
  private setupEventHandlers(): void {
    ...
    // Listen for mixer connection events
    this.tallyHub.on('mixer:connected', (mixerData: any) => {
      this.broadcastToAdminClients({
        type: 'mixer:connected',
        data: mixerData
      });
    });

    // Listen for mixer disconnection events
    this.tallyHub.on('mixer:disconnected', (mixerData: any) => {
      this.broadcastToAdminClients({
        type: 'mixer:disconnected',
        data: mixerData
      });
    });

    // Listen for mixer error events
    this.tallyHub.on('mixer:error', (mixerData: any) => {
      this.broadcastToAdminClients({
        type: 'mixer:error',
        data: mixerData
      });
  ...
  public broadcastToAdminClients(message: any): void {
    const adminClients = Array.from(this.clients.values()).filter(client => 
      client.device.name.includes('Admin')
    );
    
    if (adminClients.length > 0) {
      console.log(`📡 Broadcasting to ${adminClients.length} admin client(s):`, message.type);
    }
